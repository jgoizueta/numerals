...
module Numerals
  ...
  class NeutralNum
    ...
    def to_numeral
      ...
      if special?
        ...
        case special
          when :nan
            n.special = :indeterminate
          when :inf
            if sign=='-'
              n.special = :posinfinity
            else
              n.special  :neginfinity
            end
          else
            n = nil
        end
        ...
        n.digits = text_to_digits(digits)
        n.pnt_i = dec_pos
        n.rep_i = rep_pos
        n.sign = sign=='-' ? -1 : +1
      end
      n.normalize!(remove_trailing_zeros: !inexact) # keep trailing zeros for inexact numbers
      return n
    end
    protected
    def text_to_digits(txt)
      txt.chars.map{|b| @dgs.digit_value(b)}
    end
  end
  ...
  class Numeral
    ...
    def to_NeutralNum(base_dgs=nil)
      ...
      if special
        ...
        dec_pos = pnt_i
        rep_pos = rep_i==nil ? decimals.length : rep_i
  ...
  class Fmt
    ...
    def initialize(options=nil)
      ...
      @base_radix = 10
      @base_uppercase = true
      @base_digits = DigitsDefinition[base: @base_radix, case_sensitive: !@base_uppercase]
      @show_base = false
    ...
    def get_base_digits(b=nil) # :nodoc:
      (b.nil? || b==@base_radix) ? @base_digits : DigitsDefinition[base: b, case_sensitive: !@base_uppercase]
    end
    ...
    def set!(properties={}) # :nodoc:
     ...
     if !properties[:base_radix].nil? || !properties[:base_uppercase].nil?
        ...
        properties[:base_digits] = DigitsDefinition[base: base, case_sensitive: !uppercase]
     end

...
class TestFloatConversions <  Test::Unit::TestCase # < Minitest::Test
  ...
  def test_write_special
    ...
    assert_equal Numeral.nan, Conversions.write(Float.context.nan, rounding: Rounding[:exact, base: 2])
    assert_equal Numeral.nan, Conversions.write(Float.context.nan, rounding: Rounding[:exact, base: 10])
    ...
    assert_equal Numeral.infinity, Conversions.write(Float.context.infinity, rounding: Rounding[:exact, base: 2])
    assert_equal Numeral.infinity, Conversions.write(Float.context.infinity, rounding: Rounding[:exact, base: 10])
    ...
    assert_equal Numeral.infinity(-1), Conversions.write(Float.context.infinity(-1), rounding: Rounding[:exact, base: 2])
    assert_equal Numeral.infinity(-1), Conversions.write(Float.context.infinity(-1), rounding: Rounding[:exact, base: 10])
  ...
  def test_write_read_exact
    assert_equal Numeral[1,point:1], Conversions.write(1.0, rounding: Rounding[:exact, base: 10])
    assert_equal Numeral[1,point:1, sign: -1], Conversions.write(-1.0, rounding: Rounding[:exact, base: 10])

    assert_equal Numeral[1,point:1, base: 2], Conversions.write(1.0, rounding: Rounding[:exact, base: 2])
    assert_equal Numeral[1,point:1, sign: -1, base: 2], Conversions.write(-1.0, rounding: Rounding[:exact, base: 2])

    [0.1, 0.01, 0.001, 1/3.0, 10/3.0, 100/3.0, Math::PI, 0.5, 123.0, 123.45, 1.23E32, 1.23E-32].each do |x|
      [x, -x].each do |y|
        numeral = exact_decimal(y)
        tmp = Conversions.write(y, rounding: Rounding[:exact, base: 10])
        assert_equal numeral, Conversions.write(y, exact: true, rounding: Rounding[:exact, base: 10]), "#{y} to base 10 exact numeral"
